...
export default function Messenger({ currentUserId }) {
  ...
  const getOwnMessageReadStatus = useCallback(
    (message) => {
      ...
      const readByAll = peerMembers.every((member) => {
  ...
  const handleLeaveRoom = async () => {
    ...
    if (
      ...
      showToast(
        '다른 참여자가 없는 그룹방은 방장 권한을 이전할 수 없어 나갈 수 없습니다.',
        'warning',
      );
      return;
    }
  ...
  const renderRoomInfoContent = ({ manageable = false } = {}) => (
    <Box sx={{ minWidth: 0 }}>
      ...
      <Divider sx={{ mt: 1.4, mb: 1.2 }} />
      <Button
        fullWidth
        size="small"
        variant="outlined"
        color="error"
        startIcon={leavingRoom ? <CircularProgress size={14} color="inherit" /> : <CloseRoundedIcon />}
