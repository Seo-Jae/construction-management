...
export const createHouseholdQuantityDefinitions = ({
  ...
  const processes = (Array.isArray(processOptions) ? processOptions : [])
    ...
    .map((processName) => {
      ...
      const optionAdjustmentByType = optionRows.reduce((result, row) => {
        const rowTotal = (Number(row.quantity) || 0) * row.unitCount;
      ...
      const adjustmentTotal = baseRows.reduce(
...
export const saveHouseholdQuantityWorkbook = async ({
  ...
  definitions.processes.forEach((process) => {
    ...
    let rowNumber = 6;
    const optionDataStartRow = process.baseRows.length + 9;
    const optionDataEndRow =
