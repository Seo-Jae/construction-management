    ...
    <Box sx={{flex:1,minHeight:0,display:'grid',gridTemplateColumns:{xs:'1fr',lg:'345px minmax(0,1fr)'},gap:1}}>
      ...
      <Paper variant="outlined" sx={{minWidth:0,minHeight:0,overflow:'auto',borderColor:'#dbe3ed'}}>
        ...
        <Box sx={{p:1.5}}><Stack spacing={1.2}>
          <Alert severity="info" sx={{py:.25}}>가이드는 <strong>실제 시스템 화면 캡처</strong>를 기준으로 만듭니다. <strong>표시 편집</strong>에서 번호/박스와 설명박스를 이미지 안에 배치하고, 작업 순서는 <strong>번호 연결 화살표</strong>로 이어주세요. 번호와 관계없는 위치끼리는 <strong>지점 연결 화살표</strong>로 시작점과 도착점을 각각 클릭해 연결할 수 있습니다. 번호 원과 설명박스는 각각 이동할 수 있고 선 두께도 조절할 수 있습니다.</Alert>
          ...
          {normalizeGuideSections(editor.draft_content).map((section,index)=>{
            ...
            const numberedAnnotations=annotations.filter((item)=>item.type!=='connector'&&item.type!=='pointConnector');
