...
const getItemDisplayDetail = (item) => {
  ...
  const route = [item.origin, item.destination].filter(Boolean).join(' → ');
  const time = item.destination_time ? String(item.destination_time).slice(0, 5) : '';
  return [route || item.description || '-', time].filter(Boolean).join(' ');
};
          ...
          <TableBody>
            {rows.length === 0 ? (
              ...
              rows.map((item) => {
                ...
                const tollDetail = isToll
                  ? [item.destination || item.description, item.destination_time].filter(Boolean).join(' ')
                  : '';
                return (
                  <TableRow key={item.clientId}>
                    ...
                    <TableCell sx={cellSx}>{tollDetail}</TableCell>
          ...
          <>
            <Box
              sx={{
                ...
                gridTemplateColumns: {
                  xs: '1fr',
                  sm: 'minmax(120px, 0.85fr) minmax(120px, 0.85fr) minmax(150px, 1fr) minmax(180px, 1.1fr)',
                },
                gap: 1,
              }}
            >
...
function TollPdfImportDialog({
  ...
  const applyBulkOrigin = () => {
    const value = bulkOrigin.trim();
    if (!value) {
      setErrorText('일괄 적용할 출발지를 입력해주세요.');
      return;
    }
    onRowsChange(rows.map((row) => (row.selected ? { ...row, origin: value } : row)));
    setErrorText('');
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      ...
      <DialogContent dividers sx={{ p: 1.5 }}>
        <Stack spacing={1.1}>
          <Alert severity="info">
            <strong>{fileName}</strong>에서 SM하이플러스 영수증 {rows.length}건을 읽었습니다. 자동 입력된 내용을 확인하고 출발지만 작성해주세요.
          </Alert>
          {warnings.length > 0 && (
          ...
          <Paper variant="outlined" sx={{ p: 1, bgcolor: '#f8fafc', boxShadow: 'none' }}>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.8} alignItems={{ sm: 'center' }}>
              <TextField
                ...
                sx={{ flex: 1 }}
              />
              <Button variant="outlined" onClick={applyBulkOrigin} sx={{ whiteSpace: 'nowrap' }}>
                선택 항목에 적용
              </Button>
            </Stack>
          </Paper>
...
function ExpenseEditor({ userProfile, editingDocument, onBack, onSaved }) {
  ...
  const handleConfirmPdfImport = () => {
    ...
    setMessage({ severity: 'success', text: `통행료 ${importedItems.length}건을 사용내역 목록에 추가했습니다.` });
  };
    ...
    <Box sx={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', gap: 1.2 }}>
      ...
      >
        ...
        >
          ...
          <SectionTitle
            title="사용내역"
            subtitle="등록된 내역은 목록으로 표시됩니다. 직접 입력은 내역 추가, 하이패스는 통행료 PDF 업로드를 사용합니다."
            action={(
              <Stack direction="row" spacing={0.7}>
                <Button
                  ...
                  size="small"
                  variant="outlined"
                  startIcon={parsingPdf ? <CircularProgress size={14} /> : <UploadFileRoundedIcon />}
                  disabled={parsingPdf}
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item, index) => (
                    ...
                    >
                      ...
                      <TableCell sx={{ fontSize: '0.74rem', color: '#334155' }}>
                        <Stack direction="row" spacing={0.6} alignItems="center" flexWrap="wrap">
                          <Typography component="span" sx={{ fontSize: '0.74rem' }}>
