const fs = require('fs');
const path = require('path');

const VERSION = 'v52.07';
const projectRoot = process.cwd();
const releaseRoot = __dirname;

const relativePaths = [
  path.join('src', 'Dashboard.jsx'),
  path.join('src', 'theme.js'),
  path.join('src', 'main.jsx'),
  path.join('src', 'components', 'KoreanDatePicker.jsx'),
  path.join('src', 'components', 'KoreanMonthSelect.jsx'),
  path.join('src', 'page', 'DailyReport.jsx'),
  path.join('src', 'page', 'DrawingQuantityAnalysis.jsx'),
  path.join('src', 'page', 'ExpenseResolution.jsx'),
  path.join('src', 'page', 'LaborContractManagement.jsx'),
  path.join('src', 'page', 'LaborCostManagement.jsx'),
  path.join('src', 'page', 'MainDashboard.jsx'),
  path.join('src', 'page', 'MaterialInputStatus.jsx'),
  path.join('src', 'page', 'OrganizationChart.jsx'),
  path.join('src', 'page', 'ProgressClaimManagement.jsx'),
  path.join('src', 'page', 'ProgressInput.jsx'),
  path.join('src', 'page', 'ProposalReportEditor.jsx'),
  path.join('src', 'page', 'UserManagement.jsx'),
  path.join('src', 'page', 'WeeklyOverviewArchive.jsx'),
];

if (!fs.existsSync(path.join(projectRoot, 'package.json')) || !fs.existsSync(path.join(projectRoot, 'src'))) {
  console.error('\n[중단] package.json과 src 폴더가 있는 프로젝트 최상위 폴더에서 실행해주세요.\n');
  process.exit(1);
}

for (const relativePath of relativePaths) {
  const sourcePath = path.join(releaseRoot, relativePath);
  if (!fs.existsSync(sourcePath)) {
    console.error(`\n[중단] 누적 배포 파일을 찾을 수 없습니다.\n- ${sourcePath}\n`);
    process.exit(1);
  }
}

const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
const backupRoot = path.join(projectRoot, `backup_${VERSION}_${timestamp}`);
const backupFiles = [];

try {
  for (const relativePath of relativePaths) {
    const sourcePath = path.join(releaseRoot, relativePath);
    const targetPath = path.join(projectRoot, relativePath);

    if (fs.existsSync(targetPath)) {
      const backupPath = path.join(backupRoot, relativePath);
      fs.mkdirSync(path.dirname(backupPath), { recursive: true });
      fs.copyFileSync(targetPath, backupPath);
      backupFiles.push(backupPath);
    }

    fs.mkdirSync(path.dirname(targetPath), { recursive: true });
    fs.copyFileSync(sourcePath, targetPath);
  }

  console.log('\n=====================================================');
  console.log('사내시스템 v52.07 누적배포 적용 완료');
  console.log('기준: 운영 v52.00.2 -> 기능 v52.01 ~ v52.06');
  console.log('=====================================================');
  console.log('\n[적용 파일]');
  relativePaths.forEach((p) => console.log(`- ${p}`));
  console.log('\n[자동 백업]');
  console.log(`- ${backupRoot}`);
  console.log('\n[중요] Supabase SQL은 별도로 실행해야 합니다.');
  console.log('- release_v52.07\\Supabase에서_실행\\00_v52.00.2부터_v52.06_누적실행.sql');
  console.log('\n다음 순서');
  console.log('1. npm run build');
  console.log('2. 주요 화면 확인');
  console.log('3. 변경 파일만 git add');
  console.log('4. git commit / git push');
  console.log('5. Vercel Production Ready 및 운영 화면 확인\n');
} catch (error) {
  console.error(`\n[적용 실패] ${error.message}\n`);
  process.exit(1);
}
