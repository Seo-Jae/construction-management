// v52.48.5.44.49 시스템 가이드 별도 팝업 - 화면 내 설명박스 + 번호 연결 화살표
import { getGuideBadgePosition, normalizeGuideAnnotations, normalizeGuideSections } from '../config/guideCatalog.js';
...
const annotationGraphicHtml = (annotations, uid) => {
  ...
  const points = (item) => {
    const from = map.get(item.fromId); const to = map.get(item.toId);
  ...
  const callouts = items.filter((item)=>item.type!=='connector' && item.showLabel && (item.title||item.description)).map((item)=>`<div class="ann-callout" style="left:${item.labelX}%;top:${item.labelY}%;width:${item.labelWidth}%;border:2px solid ${escapeHtml(item.color)}"><div class="ann-callout-title" style="color:${escapeHtml(item.color)}">${escapeHtml(item.number)}. ${escapeHtml(item.title||'안내')}</div>${item.description?`<div class="ann-callout-desc">${textToHtml(item.description)}</div>`:''}</div>`).join('');
...
export const renderSystemGuidePopup = (popup, { label='가이드', breadcrumb=label, title=label, summary='', sections=[], published=false, message='' }) => {
  ...
  const overview = withImages.length ? `<section class="overview"><h2 class="overview-title">사용 순서 한눈에 보기</h2><div class="overview-sub">실제 화면의 번호·설명박스·연결 화살표를 따라 순서대로 확인하세요.</div><div class="overview-grid">${withImages.map((section,index)=>`<div class="overview-card">${imageHtml(section,`ov-${index}`,true)}<div class="overview-caption"><div class="overview-num">${index+1}</div><div><div class="overview-name">${escapeHtml(section.title || `화면 ${index+1}`)}</div>${(section.imageCaption||section.content)?`<div class="overview-desc">${textToHtml(section.imageCaption||section.content)}</div>`:''}</div></div></div>`).join('')}</div></section>` : '';
