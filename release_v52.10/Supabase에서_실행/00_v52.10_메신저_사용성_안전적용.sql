-- ============================================================
-- 사내시스템 v52.10 / 메신저 설치없는 웹 별도창 + 사용성 개선
-- 기준: 운영 v52.08.4 이상
--
-- 포함되는 DB 기능
--   1) 사용자별 메신저 글자크기 저장
--   2) 그룹 방장 직접 위임
--   3) 방장 나가기 시 명시적 위임 대상 선택
--   4) 1인 그룹방 / 일반 참여자 / 1:1 나가기 유지
--
-- v52.09 SQL에서 장시간 Running이 발생했던 환경을 고려해
-- messenger_leave_room(uuid) 기존 함수를 DROP하지 않는다.
-- 새 프런트는 messenger_leave_room(uuid, uuid) 시그니처를 사용한다.
-- 따라서 운영 중인 기존 메신저 호출을 막지 않고 안전하게 교체할 수 있다.
-- ============================================================
...
begin
  ...
  v_scale := greatest(
    0.85,
    least(1.30, round(coalesce(p_font_scale, 1.00)::numeric, 2))
  );
...
-- ------------------------------------------------------------
-- 3. 대화방 나가기
--    - v52.08.4의 1인 그룹방 나가기 유지
--    - 다른 참여자가 있는 그룹방 방장은 위임 대상을 직접 선택
--    - 기존 messenger_leave_room(uuid)는 DROP하지 않는다.
-- ------------------------------------------------------------
create or replace function public.messenger_leave_room(
  p_room_id uuid,
  p_transfer_owner_id uuid
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if v_room_type = 'group' and v_owner_id = v_current_user then
    ...
    -- 혼자 남은 그룹방은 위임 대상 없이 바로 나갈 수 있다.
...
select
  ...
  to_regprocedure('public.messenger_leave_room(uuid,uuid)') as messenger_leave_room_v2,
  to_regprocedure('public.messenger_leave_room(uuid)') as messenger_leave_room_legacy;
