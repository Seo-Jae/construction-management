...
export default function AttendanceQrDisplay() {
  ...
  useEffect(() => {
    document.title = '욱림건설 출·퇴근 QR';
  }, []);

  useEffect(() => {
    if (!displayToken) {
      setErrorMessage('QR 표시 세션이 없습니다. 근태관리에서 다시 열어주세요.');
      return undefined;
    }
    ...
    >
      ...
      >
        ...
        <Alert severity="info" sx={{ mt: 2, textAlign: 'left', fontSize: '0.76rem' }}>
          이 창은 출·퇴근 QR만 표시하며 회원정보·근태기록·관리 메뉴에는 접근할 수 없습니다. 담당자 화면이 로그아웃되어도 표시 세션 만료 전까지 계속 작동합니다.
        </Alert>
      </Paper>
    </Box>
  );
