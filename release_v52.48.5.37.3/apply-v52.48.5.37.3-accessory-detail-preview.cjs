const fs = require('fs');
const path = require('path');

const VERSION = 'v52.48.5.37.3';
const ROOT = process.cwd();
const TARGET = path.join(ROOT, 'src', 'utils', 'technicalImageSheetEditor.js');
const REPLACEMENT = path.join(
  __dirname,
  'files',
  'src',
  'utils',
  'technicalImageSheetEditor.js',
);

function fail(message) {
  console.error(`[적용 중단] ${message}`);
  process.exitCode = 1;
}

if (!fs.existsSync(TARGET)) {
  fail(`대상 파일을 찾을 수 없습니다: ${TARGET}`);
} else if (!fs.existsSync(REPLACEMENT)) {
  fail('교체파일을 찾을 수 없습니다. ZIP을 다시 풀어주세요.');
} else {
  const current = fs.readFileSync(TARGET, 'utf8').replace(/\r\n/g, '\n');

  if (current.includes('v52.48.5.37.3')) {
    console.log(`[${VERSION}] 이미 적용되어 있습니다.`);
  } else if (
    !current.includes('// v52.48.5.37.2')
    || !current.includes('height: min(30vh, 285px)')
  ) {
    fail(
      '현재 technicalImageSheetEditor.js가 v52.48.5.37.2 기준과 다릅니다. '
      + '기존 변경을 보호하기 위해 자동 교체하지 않았습니다.',
    );
  } else {
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const backupDir = path.join(
      ROOT,
      `backup_${VERSION}_${stamp}`,
      'src',
      'utils',
    );

    fs.mkdirSync(backupDir, { recursive: true });
    fs.copyFileSync(
      TARGET,
      path.join(backupDir, 'technicalImageSheetEditor.js'),
    );

    fs.copyFileSync(REPLACEMENT, TARGET);

    console.log(`[${VERSION}] 적용 완료`);
    console.log('- 선택 번호/명칭 상세이미지: 우측 패널 전체 폭으로 확대');
    console.log('- 세로형 이미지: 제품사진 + 하단 상세설명이 함께 보이도록 250px 제한 제거');
    console.log('- VIEW에서 이미지 바깥 흰 여백 자동 최소화 (가능 시)');
    console.log('- 원본 Storage 이미지는 수정하지 않음');
    console.log('- 전체보기 목록 클릭: 작은 compact 미리보기 유지');
    console.log('- 기존 번호 선택/부속자재 연결/지시선 편집/DB 기능 유지');
    console.log('- SQL 실행 없음');
    console.log(`- 백업: ${path.relative(ROOT, backupDir)}`);
  }
}
