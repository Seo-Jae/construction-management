    ...
    <Box sx={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', gap: 1.2 }}>
      ...
      >
        ...
        >
          ...
          <SectionTitle
            title="사용내역"
            subtitle="유류대는 출발지와 도착지를 구분해서 입력합니다."
            action={(
              ...
              return (
                ...
                >
                  ...
                  <Box
                    sx={{
                      display: 'grid',
                      gridTemplateColumns: { xs: '1fr', sm: '125px 145px 1fr 130px' },
                      gap: 0.8,
                    }}
                  >
                    <TextField
                      ...
                      onChange={(event) => handleItemChange(item.clientId, 'expense_date', event.target.value)}
                      InputLabelProps={{ shrink: true }}
                      inputProps={{ min: `${month}-01`, max: `${month}-${pad(getDaysInMonth(month))}` }}
                    />
                    <TextField
                      select
                      label="지출항목"
