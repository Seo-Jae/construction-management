        ...
        >
          ...
          {canShareAllProjects ? (
            ...
            >
              ...
              >
                체크하면 한 현장에서 등록한 이슈가 모든 현장의 Main 캘린더에 표시됩니다.
              </Typography>
            </Box>
          ) : (
            <Typography
            ...
            >
              이 이슈는 현재 현장에만 등록됩니다. 전체현장 공유 권한은 회원관리에서 최고관리자가 부여할 수 있습니다.
            </Typography>
          )}
        </Box>
      </DialogContent>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {viewYear}년 {viewMonth + 1}월 주요 이슈
          </Typography>
...
export default function MainDashboard({
  ...
  const loadCalendarIssues = useCallback(async () => {
    ...
    try {
      ...
      console.error('Main 캘린더 주요 이슈 조회 오류:', error);
      setCalendarIssues([]);
      setCalendarIssueAccess(EMPTY_CALENDAR_ISSUE_ACCESS);
      setCalendarIssuesErrorMessage(
        error?.message || '주요 이슈를 불러오지 못했습니다.',
      );
    } finally {
  ...
  const handleSaveCalendarIssue = async () => {
    ...
    try {
      ...
      console.error('Main 캘린더 주요 이슈 저장 오류:', error);
      setToast({
        open: true,
        severity: 'error',
        message:
          error?.message || '주요 이슈를 저장하지 못했습니다.',
      });
    } finally {
  ...
  const handleDeleteCalendarIssue = async (issue) => {
    const confirmed = window.confirm(
      `${String(issue.issue_date || '').slice(0, 10)} 주요 이슈를 삭제할까요?`,
    );
    ...
    try {
      ...
      setToast({
        open: true,
        severity: 'success',
        message: '주요 이슈를 삭제했습니다.',
      });
      await loadCalendarIssues();
    } catch (error) {
      console.error('Main 캘린더 주요 이슈 삭제 오류:', error);
      setToast({
        open: true,
        severity: 'error',
        message:
          error?.message || '주요 이슈를 삭제하지 못했습니다.',
      });
    }
  };
