...
export default function ProposalReportEditor({
  userProfile,
  editingDocument,
  onBackToList,
}) {
  ...
  const [title, setTitle] = useState(storedPayload.title || '');
  const [reportDate, setReportDate] = useState(
    storedPayload.reportDate || getTodayKey(),
  );
  const [reportLines, setReportLines] = useState(() => {
    const lines = Array.isArray(storedPayload.reportLines)
      ? [...storedPayload.reportLines]
      : createInitialLines();

    while (lines.length < MIN_REPORT_LINES) lines.push('');
    return lines.slice(0, MAX_REPORT_LINES);
  });
  const [itemName, setItemName] = useState(storedPayload.itemName || '');
  const [amount, setAmount] = useState(
    storedPayload.amount === null || storedPayload.amount === undefined
      ? ''
      : String(storedPayload.amount),
  );
  const [note, setNote] = useState(storedPayload.note || '');
  ...
  const approvalReportKey = useMemo(
    () =>
      editingDocument?.report_key ||
      `proposal:${reportDate}:${documentInstanceKey}`,
    [documentInstanceKey, editingDocument?.report_key, reportDate],
  );
  ...
  const handleSaveDraft = async ({ silent = false } = {}) => {
    ...
    try {
      ...
      console.error('품의 보고 저장 실패:', error);
      setDownloadError(error?.message || '작성 내용을 저장하지 못했습니다.');
      return null;
    } finally {
      setSaving(false);
    }
      ...
      >
        <Box sx={{ p: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Button
            ...
            disabled={approvalStatusLoading || approvalLocked || saving}
            sx={{
