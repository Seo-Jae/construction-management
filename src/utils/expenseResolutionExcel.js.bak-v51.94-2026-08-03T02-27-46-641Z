...
const setCalendarCellColors = (cell, fillColor, fontColor) => {
  // 원본 양식은 여러 셀이 같은 style 객체를 공유합니다. 특정 셀의 fill만
  // 바꾸면 같은 style을 쓰는 다른 줄의 색까지 다시 바뀔 수 있으므로,
  // 셀마다 전체 style을 먼저 분리한 뒤 색상을 적용합니다.
...
const parseRouteDescription = (value) => {
  ...
  if (!raw.startsWith(TOLL_META_PREFIX)) {
    return { distanceKm: sanitizeDistanceInput(raw), fuelAmount: 0 };
  }
  try {
    const parsed = JSON.parse(raw.slice(TOLL_META_PREFIX.length));
    return {
      distanceKm: sanitizeDistanceInput(parsed?.distanceKm),
      fuelAmount: toNumber(parsed?.fuelAmount),
    };
  } catch (error) {
    return { distanceKm: '', fuelAmount: 0 };
  }
};

const getFuelAmount = (item) => {
  if (item?.category !== 'toll') return 0;
  if (item?.fuel_amount !== undefined && item?.fuel_amount !== null && item?.fuel_amount !== '') {
    return toNumber(item.fuel_amount);
  }
  return parseRouteDescription(item?.description).fuelAmount;
};

const getItemTotalAmount = (item) => toNumber(item?.amount) + getFuelAmount(item);

const getDistanceText = (item) => {
  const distance = item?.distance_km
    ? sanitizeDistanceInput(item.distance_km)
    : parseRouteDescription(item?.description).distanceKm;
  return distance ? `${distance}km` : '';
};

const buildTollText = (item) => {
  const destination = cleanRoutePlace(item?.destination);
  const time = String(item?.destination_time || '').slice(0, 5);
  return [destination, time].filter(Boolean).join(' ') || '통행료';
};
...
const buildCalendarRouteText = (item) => {
  const route = [cleanRoutePlace(item.origin), cleanRoutePlace(item.destination)]
    .filter(Boolean)
    .join('→');
  return route || '이동';
...
const getCalendarEntryText = (item) => {
  ...
  if (item.category === 'fuel' || item.category === 'toll') {
    return `${buildCalendarRouteText(item)}(유류비)`;
  }
...
const normalizeItems = (items) =>
  [...(items || [])]
    .map((item, index) => ({
      ...item,
      distance_km: item?.distance_km || parseRouteDescription(item?.description).distanceKm,
      fuel_amount: getFuelAmount(item),
      amount: toNumber(item.amount),
      sort_order: item.sort_order ?? index,
    }))
    .filter((item) => item.expense_date && getItemTotalAmount(item) > 0)
    .sort((first, second) => {
...
const calculateTotals = (items) => {
  ...
  items.forEach((item) => {
    ...
    if (item.category === 'toll') totals.fuel += getFuelAmount(item);
  });
  return totals;
};
...
const populateCoverSheet = ({ sheet, month, claimDate, claimantName, items }) => {
  ...
  coverRows.forEach((rowData, index) => {
    ...
    if (rowData.toll) {
      ...
      sheet.getCell(`C${rowNumber}`).value = getFuelAmount(rowData.toll) || null;
      sheet.getCell(`D${rowNumber}`).value = buildTollText(rowData.toll);
...
const updateCalendarInstructions = (sheet) => {
  const instructions = {
    ...
    Q6: '2. 통행료 PDF 내역은 도착지와 도착시간을 통행료 내용칸에 작성',
    Q7: '3. PDF 업로드 시 입력한 유류비는 같은 행의 유류비 금액칸에 작성',
...
const populateCalendarSheet = ({ sheet, month, items }) => {
  ...
  entriesByDate.forEach((entries, dateValue) => {
    ...
    entries.slice(0, 4).forEach((entry, index) => {
      ...
      cell.value = entry;
      cell.alignment = { ...cell.alignment, vertical: 'middle', horizontal: 'left', wrapText: false };
    });
