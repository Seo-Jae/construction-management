...
const alreadyApplied =
  ...
  historical.includes('sourceProcess ? normalizeText(sourceProcess) : job');

if (alreadyApplied) {
  console.log('[v52.29] 이미 프로그램 파일이 적용된 상태입니다.');
  process.exit(0);
}
...
const backupDir = path.join(ROOT, `backup_v52.29_${stamp}`);
