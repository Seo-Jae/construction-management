...
const backupDir = path.join(root, `backup_v52.17.2_${stamp}`);
...
const files = [
  {
    ...
    acceptedBaseSha256: [
      'b33de1bafa682ce5e5815ab76e41a45972adfbc0b3c2907c5231203b1c77e175',
    ],
    newSha256: 'f540ff0335aa3ea19e30d82c8f44e4da518bd3dbdf6a74207a8fb2124db2c682',
  },
];
...
for (const item of files) {
  ...
  if (!item.acceptedBaseSha256.includes(currentHash)) {
    fail(
      `${item.target} 파일이 v52.17.1 배포 기준과 다릅니다.\n` +
...
if (needsApply.length === 0) {
  console.log('[v52.17.2] 이미 프로그램 파일이 적용되어 있습니다.');
  process.exit(0);
}
