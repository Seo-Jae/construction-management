    ...
    try {
      ...
      setErrorMessage(
        ...
        message.includes('admin_list_user_access_v2')
          ? '회원권한 2단계 SQL이 아직 적용되지 않았습니다. 제공된 Supabase SQL을 먼저 실행해주세요.'
      ...
      return {
        ...
        [userId]: {
          ...
          projectAccess: normalizeProjectAccess(
            normalizeProjectSelection(values, current.projectNames),
              ...
              <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto', p: { xs: 1.2, md: 1.6 }, bgcolor: '#f8fafc' }}>
                ...
                <Paper variant="outlined" sx={{ mt: 1.2, p: 1.4, borderColor: '#e2e8f0', boxShadow: 'none', bgcolor: '#ffffff' }}>
                  ...
                  <Alert severity="info" sx={{ mt: 1, py: 0, fontSize: '0.7rem' }}>
                    이번 단계에서는 템플릿과 현장범위를 저장합니다. 기존 메뉴 접근 방식은 유지되며 실제 메뉴별 차단은 다음 단계에서 연결합니다.
                  </Alert>
                </Paper>
                ...
                <Box sx={{ mt: 1.2, display: 'grid', gridTemplateColumns: { xs: '1fr', xl: 'repeat(2, minmax(0, 1fr))' }, gap: 1.2 }}>
                  <Paper variant="outlined" sx={{ p: 1.4, borderColor: '#e2e8f0', boxShadow: 'none' }}>
                    <SectionTitle
                      number="4"
                      title="세부권한"
                      description="DB 권한표 구성 완료 · 다음 단계에서 개인별 허용·차단 화면을 연결합니다."
                      ready={false}
                    />
                  </Paper>
                  <Paper variant="outlined" sx={{ p: 1.4, borderColor: '#e2e8f0', boxShadow: 'none' }}>
                    <SectionTitle
                      number="5"
                      title="특수권한"
                      description="민감 권한 DB 구성 완료 · 다음 단계에서 개별 승인 스위치를 연결합니다."
                      ready={false}
                    />
                  </Paper>
                </Box>
