...
-- =========================================================
-- v52.40
-- 월별 노임작성 = 근로자 조회/선별/개인정보 Excel 다운로드 준비
--
-- 웹에서 노임을 작성하지 않는다.
-- 실제 출역일자/일급/노임금액/지급공제는 Excel에서 작성한다.
--
-- v52.36에서 만든 출역/금액 컬럼은 호환성 때문에 삭제하지 않는다.
-- =========================================================
...
begin
  if to_regprocedure(
    ...
    raise exception
      'v52.37 Excel 생성 사전검증 함수가 없습니다.';
  end if;

  if to_regprocedure(
    'public.labor_monthly_snapshot_v52_39(text,text)'
  ) is null then
    raise exception
      'v52.39 다운로드 인증 snapshot 함수가 없습니다.';
  end if;
end;
$$;
...
begin
  ...
  v_hash :=
    encode(
      extensions.digest(
        convert_to(coalesce(v_material, ''), 'UTF8'),
        'sha256'
      ),
      'hex'
    );
...
comment on function public.labor_monthly_export_readiness_v52_37(text, text) is
  'v52.40: 개인정보/공종 기준 Excel 준비검사. 출역/일급/노임금액은 검사하지 않음.';

comment on function public.labor_monthly_snapshot_v52_39(text, text) is
  'v52.40: 근로자 선별/순서/공종/비고/개인정보 변경만 다운로드 인증 snapshot에 binding.';
