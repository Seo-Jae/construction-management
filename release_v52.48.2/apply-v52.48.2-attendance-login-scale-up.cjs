...
function replaceUnique(source, oldText, newText, label) {
  ...
  if (first < 0) fail(`${label}: 기준 코드를 찾지 못했습니다.`);
...
if (source.includes('data-attendance-login-scale="v52.48.2"')) {
  console.log('[v52.48.2] 이미 적용된 상태입니다.');
  process.exit(0);
}

if (
  !source.includes('data-attendance-login-ui') ||
  !source.includes("cleanLogin={mode === 'login'}")
) {
  fail('v52.48.1 로그인 UI가 먼저 반영되어 있어야 합니다.');
}
...
console.log('\n[v52.48.2 적용 완료]');
console.log('- 로그인 좌우 여백 약 32px → 20px');
console.log('- 로그인 시작 위치 위로 이동');
console.log('- W / 로그인 제목 확대');
console.log('- 휴대폰 / 비밀번호 입력창 확대');
console.log('- 로그인 / 가입신청 / 관리자모드 확대');
console.log('- v52.48 관리자 QR 기능 유지');
