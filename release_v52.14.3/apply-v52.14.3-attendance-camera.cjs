...
const acceptedHashes = [
  'd1f680f647a9a731f945811687097786c3f5a4e931ca27d64ed6948c715fd654',
];
...
if (!fs.existsSync(path.join(projectRoot, 'package.json'))) {
  console.error('[v52.14.3] package.json을 찾을 수 없습니다. release_v52.14.3 폴더를 프로젝트 최상위에 두고 실행해주세요.');
  process.exit(1);
}
...
if (!fs.existsSync(sourcePath) || !fs.existsSync(targetPath)) {
  console.error(`[v52.14.3] 적용 파일을 찾을 수 없습니다: ${relativePath}`);
  process.exit(1);
}
...
if (currentHash === releaseHash) {
  console.log('[v52.14.3] 카메라 무한대기 보완이 이미 적용되어 있습니다.');
  console.log('- 다음 단계: npm run build');
  process.exit(0);
}

if (!acceptedHashes.includes(currentHash)) {
  console.error(`[v52.14.3] 현재 ${relativePath} 파일이 검증된 v52.14.2 기준과 다릅니다.`);
...
const backupPath = path.join(projectRoot, `backup_v52.14.3_${stamp}`, relativePath);
fs.mkdirSync(path.dirname(backupPath), { recursive: true });
fs.copyFileSync(targetPath, backupPath);
fs.copyFileSync(sourcePath, targetPath);

console.log('[v52.14.3] 근태 QR 카메라 무한대기 보완 적용 완료');
